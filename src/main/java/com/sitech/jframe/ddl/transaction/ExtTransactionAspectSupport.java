package com.sitech.jframe.ddl.transaction;
 ...
 *
 */
public abstract class ExtTransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, Class targetClass, final InvocationCallback invocation)
		...
		// 在 Spring 事务创建前 根据 TransactionAttribute 确定读写属性
		final ReadWriteSeparate readWriteSeparate = prepareDynamicDataSourceContext(txAttr);
		if (txAttr == null || !(tm instanceof CallbackPreferringPlatformTransactionManager)) {
			...
			try {
				...
				cleanupReadWriteSeparate(readWriteSeparate);
			}
	 ...
	 * @param txAttr
	 */
	private ReadWriteSeparate prepareDynamicDataSourceContext(TransactionAttribute txAttr) {
		
		if (!(txAttr instanceof RuleBasedTransactionAttribute)) {
			throw new IllegalArgumentException("无效参数 RuleBasedTransactionAttribute");
		}
		
		ReadWriteSeparate readWriteSeparate = new ReadWriteSeparate();
		boolean readOnly = txAttr.isReadOnly();
		if (readOnly) {	// 如果有 readOnly 属性
			// 设置 TransactionAttribute 不支持事务
			((RuleBasedTransactionAttribute)txAttr).setPropagationBehavior(Propagation.NOT_SUPPORTED.value());
			
			// 设置读数据库
			readWriteSeparate.newReadWriteEnum(ReadWriteEnum.READ);
		} else {
			readWriteSeparate.newReadWriteEnum(ReadWriteEnum.WRITE);
		}
	...
	protected void cleanupReadWriteSeparate(ReadWriteSeparate readWriteSeparate) {
